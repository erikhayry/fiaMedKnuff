	...
	.factory('boardFactory', ['settings', 'rules',
      ...
      var _board = {},
          ...
          _getNumberOfTiles = function(numberOfPlayers){
            if(numberOfPlayers < 5) return 4 * settings.tilesPerPlayer;
          ...
          _canLeaveNest = function(diceValue){
            for (var i = 0; i < rules.canLeaveNestValues.length; i++) {
              if(diceValue === rules.canLeaveNestValues[i]) return true;
            }
            return false;
          },

          _getOffset = function(playerId){
            if(_board.players.length == 2) return playerId * (2 * settings.tilesPerPlayer);
            else return playerId * settings.tilesPerPlayer;
          },

          _move = function(playerId, tokenId, from, diceValue){
            var _newTrackIndex = from + diceValue;

            if(!_board.track[_newTrackIndex]) _board.track[_newTrackIndex] = {};
            
            if(playerId in _board.track[_newTrackIndex]) _board.track[_newTrackIndex][playerId].push(tokenId);
            else _board.track[_newTrackIndex][playerId] = [tokenId];
        ...
        moveToTrack: function(tokenId, diceValue){
          if(!_canLeaveNest(diceValue)) return false;

          var _playerId = tokenId.slice(0, tokenId.indexOf('-')),
              _offset = _getOffset(parseInt(_playerId));

          if(rules.startAtTileDiceValue) _move(_playerId, tokenId, _offset, diceValue - 1);
          else _move(_playerId, tokenId, _offset, 0);
